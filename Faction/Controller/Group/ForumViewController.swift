 ...
 class ForumViewController: UIViewController, CLLocationManagerDelegate, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITableViewDelegate, UITableViewDataSource { // AND GroupDescriptionController
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //controller.pageContent.content = forumContent as  Attempt to sync across controllers
        ...
        if(indexPath.row == 0){ // forum cell
            let cell = tableView.dequeueReusableCell(withIdentifier: cellId3, for: indexPath) as! ForumCell
            cell.backtrace = forumContent.backtrace!
            ...
            cell.backtrace = forumContent.comments?[indexPath.row-1]["backtrace"] as! [String]
            cell.controller = controller
            ...
            cell.timeLabel.text = dateFormatter.string(from: timestampDate)
            if(forumContent.comments?[indexPath.row-1]["score"] != nil){
                cell.postValue.text = forumContent.comments?[indexPath.row-1]["score"]!["value"] as! String
            }
 ...
 class ForumCreationController: UIViewController, UITextFieldDelegate, UITextViewDelegate{
    ...
    lazy var titleField: UITextField = {
        ...
        textField.borderStyle = UITextBorderStyle.roundedRect
        textField.backgroundColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1.0);
        textField.placeholder = "Insert title here..."
        textField.translatesAutoresizingMaskIntoConstraints = false
        textField.delegate = self
        return textField
    }()
    ...
    lazy var contentField: UITextField = {
        ...
        textField.borderStyle = UITextBorderStyle.roundedRect
        textField.backgroundColor = UIColor(red: 1, green: 1, blue: 1, alpha: 1.0);
        textField.placeholder = "Add your post here..."
        textField.translatesAutoresizingMaskIntoConstraints = false
        textField.delegate = self
        return textField
    }()
    ...
    func handleConfirm(){
        ...
        FIRDatabase.database().reference().child("faction").child(factionId).child("pages").child(currentPos).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            if let dictionary = snapshot.value as? [String:AnyObject] { // fill in type with suspected type
                ...
                backtrace?.append(nextPos)
                var values = ["date": Int(Date().timeIntervalSince1970), "user": (FIRAuth.auth()?.currentUser?.uid)!, "content": self.contentField.text, "title": self.titleField.text, "score": ["value":"1", "users":[(FIRAuth.auth()?.currentUser?.uid)!:1]], "backtrace": backtrace] as [String : AnyObject]
